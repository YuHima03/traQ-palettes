...
namespace Palettes.Utils.Caching.Traq
{
    public static class StampApiExtension
    {
        ...
        public static async ValueTask<Stamp> GetCachedStampAsync(this IStampApiAsync api, IMemoryCache cache, string name, CancellationToken ct = default)
        {
            ...
            var (_, s) = (await api.GetCachedStampsAsync(cache, null, ct)).First(s => s.Value.Name == name); // GetCachedStampsAsync method updates all caches related to stamps.
        ...
        public static async ValueTask<ConcurrentDictionary<Guid, StampWithThumbnail>> GetCachedStampsAsync(this IStampApiAsync api, IMemoryCache cache, string? type, CancellationToken ct = default)
        {
            if (cache.TryGetValue<string?, ConcurrentDictionary<Guid, StampWithThumbnail>>(CacheSections.AllStamps, type, out var stamps) && stamps is not null)
            {
                return stamps;
            }
            stamps = new((await api.GetStampsAsync(null, type, ct)).Select(s => KeyValuePair.Create(s.Id, s)));
